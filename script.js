...
async function connectHA() {
    try {
        ...
        subscribeEntities(connection, (entities) => {
            ...
            // --- 1. CLIMA (TRATAMENTO DE ERRO ISOLADO) ---
            try {
                ...
                if (weather) {
                    ...
                    // Lógica para chuva no AccuWeather (tenta achar a probabilidade)
                    const prob = attrs.precipitation_probability || 
                                 (attrs.forecast && attrs.forecast[0]?.precipitation_probability) || 0;
                    document.getElementById('w-rain').innerText = prob;

                    const icons = { 
                        'sunny': '☀️', 'clear-night': '🌙', 'cloudy': '☁️', 
                        'partlycloudy': '⛅', 'rainy': '🌧️', 'pouring': '⛈️' 
                    };
                    document.getElementById('w-icon').innerText = icons[weather.state] || '🌡️';
                    document.getElementById('w-condition').innerText = weather.state.toUpperCase();
                }
            } catch (e) { console.warn("Erro no Clima:", e); }

            // --- 2. BOTÕES DE TOPO (CHIPS) ---
            ...
            MY_ENTITIES.forEach(item => {
                ...
                if (!card) {
                    ...
                    card.innerHTML = `<div class="icon">${item.icon}</div><div class="name">${item.label}</div><div class="status">...</div>`;
